...
class Embeds(object):
    def __init__(self, fname, w2v_type='fasttext', format='file'):
        if format in ('json', 'pickle'):
            ...
            self.model = KeyedVectors.load_word2vec_format(fname, binary = True)
        else:
            self.model = {}
...
if model != 'mvcnn':
    embed_dim = 300
    embeds = Embeds("/camera_data2/glove.840B.300d.txt", "glove", format="txt")
...
for i, (train_index, test_index) in enumerate(kf.split(x, y)):
    ...
    embeds = Embedding(embedding_matrix.shape[0],
                       ...
                       trainable=False,
                       input_length=max_seq_len)(text_seq_input)

    embeds = SpatialDropout1D(0.25)(embeds)
    xx = Bidirectional(CuDNNLSTM(rnn_dim, return_sequences=True))(embeds)

    capsule = Capsule(num_capsule=2 * Num_capsule, dim_capsule=2 * Dim_capsule, routings=Routings,
                      share_weights=True)(xx)
    xx = Capsule(num_capsule=Num_capsule, dim_capsule=Dim_capsule, routings=Routings,
                      share_weights=True)(capsule)

    capsule = Flatten()(xx)
    xx = Dropout(dropout_val)(capsule)

    auxiliary_input = Input(shape=(5,), name='aux_input')

    xx = Concatenate()([xx, auxiliary_input])
...
np.savetxt("/output/oof_train_capsule_glove.csv", oof_train, fmt='%.24f', delimiter=',')
...
result.to_csv("/output/capsule_glove_sub.csv", index=False)
