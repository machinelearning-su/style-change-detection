...
if model != 'mvcnn':
    embed_dim = 300
    embeds = Embeds("/camera_data2/crawl-300d-2M.vec", "fasttext", format="vec")
...
for i, (train_index, test_index) in enumerate(kf.split(x, y)):
    ...
    xx = Reshape((max_seq_len, embedding_matrix.shape[1], 1))(xx)

    pooled = []
    for i in filter_sizes:
        conv = Conv2D(64, kernel_size=(i, embedding_matrix.shape[1]), kernel_initializer='normal',
                      activation='elu')(xx)
...
np.savetxt("/output/oof_train.csv", oof_train, fmt='%.24f', delimiter=',')
...
result.to_csv("/output/2dcnn_sub.csv", index=False)